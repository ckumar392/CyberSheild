...
def check_internet():
    os.system('ping -c1 github.com > xp_net 2>&1')
    if "0% packet loss" in open('xp_net').read():
        val = 1
    else:
        val = 0
    os.system('rm xp_net > /dev/null 2>&1')
    return val
...
if args_namespace.help or (not args_namespace.update \
    ...
    print("\tTotal Number of Vulnerability Checks Skipped: "+bcolors.BOLD+bcolors.WARNING+str(xp_skipped_checks)+bcolors.ENDC)
    print("\tTotal Number of Vulnerabilities Detected    : "+bcolors.BOLD+bcolors.BADFAIL+str(len(xp_vul_list))+bcolors.ENDC)
    print("\tTotal Time Elapsed for the Scan             : "+bcolors.BOLD+bcolors.OKBLUE+display_time(int(xp_total_elapsed))+bcolors.ENDC)
    print("\n")
    print("\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+bcolors.OKBLUE+"`xp-Debug-ScanLog`"+bcolors.ENDC+" under the same directory.")
